class Api::V1::EventUsersController < Api::V1::ApplicationController
  ...
  def index #this shows all the event that you've joined
    ...
    render :json => @my_joined_events.to_json( :include => [{:event => { :include => [{:subcategory => {:only => :name}}, {:guests => { :only => :fullname }  }, {:host => { :except => [:encrypted_password, :created_at, :updated_at, :confirmation_token] }} ]}}])
